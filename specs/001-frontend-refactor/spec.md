...
| 颜色 | 使用场景 | 示例组件 |
|------|----------|----------|
| **主色 #1677ff** | 主要操作按钮、链接、选中状态、加载指示器 | 发送按钮、会话选中态、进度条 |
| **主背景 #ffffff** | 页面主体背景、卡片背景、输入框背景 | 整体页面、消息气泡、文本输入 |
| **次要背景 #f5f8fc** | 区域分隔背景、侧边栏背景、hover 状态 | 会话列表背景、思考过程背景 |
| **主文字色 #1d2129** | 标题、正文、用户消息文本 | 消息内容、会话标题 |
| **次文字色 #4e5969** | 辅助说明、时间戳、占位符 | 消息时间、空状态提示 |
| **边框色 #e5e8ef** | 分隔线、卡片边框、输入框边框 | 会话列表分隔、消息气泡边框 |

#### 对比度验证

所有文字色与背景色组合均符合 **WCAG AA 标准**（对比度 ≥ 4.5:1）:
...
### 7.1 功能验收（MVP 范围 - Phase 3 US1 核心）
- [ ] 流式对话正常工作：流式更新间隔 < 100ms，UI 线程阻塞 < 16ms（60 FPS）
- [ ] 思考过程和工具调用正确展示：所有 SSE chunk 类型正确解析和渲染
- [ ] 会话切换无数据丢失：切换耗时 < 300ms，状态完整保留
- [ ] 消息历史记录正确保存和加载：LocalStorage 读写成功率 100%
- [ ] Markdown 渲染和代码高亮正常：支持 GFM 扩展，代码高亮 100+ 语言

### 7.1.5 后续迭代范围（Phase 4-7）
- ⏳ 知识库选择功能（Phase 3 US1 后期）
- ⏳ 移动端布局正常（Phase 5 US3）
- ⏳ 页面动画流畅（Phase 6 US4）
- ⏳ 性能优化完成（Phase 7）

### 7.2 界面视觉验收（清新淡雅标准）
- [ ] 主题色匹配规范（#1677ff、#ffffff、#f5f8fc 等）
- [ ] 文字对比度 >= 4.5:1（WCAG AA 标准，使用 WebAIM Contrast Checker 验证）
- [ ] 所有按钮和交互元素在白色背景上清晰可见（最小触控区域 44x44px）
- [ ] 无使用深色主题或高饱和度配色的残留组件

### 7.3 性能验收

**测量工具**: Chrome DevTools Performance、Lighthouse CI

**测试条件**:
- 网络: Fast 3G (下行 1.6 Mbps, RTT 150ms)
- 设备: Desktop (CPU 4x slowdown)
- 缓存: 首次访问清空缓存，二次访问保留缓存

**验收标准**:

| 指标 | 目标值 | 测量方法 |
|------|--------|----------|
| 首屏加载时间 (FCP) | < 2s | Lighthouse Performance |
| 可交互时间 (TTI) | < 3.5s | Lighthouse Performance |
| 流式消息渲染延迟 | < 100ms | Performance.now() 差值 |
| 长列表滚动帧率 | >= 55 FPS | Chrome DevTools FPS Meter |
| 页面切换动画帧率 | >= 58 FPS | requestAnimationFrame 监控 |
| 长对话列表 (100+ 消息) | 渲染 < 2s，内存 < 150MB | Performance Monitor |

**性能测试场景**:
1. **场景 1**: 空缓存首次加载 → 测量 FCP、TTI
2. **场景 2**: 发送简单消息 → 测量流式响应延迟
3. **场景 3**: 加载 100 条消息的历史对话 → 测量渲染时间和滚动性能
4. **场景 4**: 页面间切换 (Chat <-> Knowledge) → 测量动画帧率
...
### 7.5 规范遵守
- [ ] 所有组件采用模块化设计，可独立测试
- [ ] 所有 API 调用使用 TypeScript 接口作为契约
- [ ] 关键交互点（XRequest callbacks、错误处理）有 console 日志
- [ ] 代码符合 AgentMind 宪章要求
